...
var Layout = /** @class */ (function () {
    ...
    */
    ...
    Layout.prototype.getNextSibling = function (currentElementBox) {
        var paragraph = currentElementBox.line.paragraph;
    ...
    Layout.prototype.layoutFootEndNoteElement = function (element) {
        ...
        if (element.footnoteType === 'Footnote') {
            ...
            if (height <= clientActiveArea.height) {
                element.blocks.forEach(function (element) {
    ...
    Layout.prototype.checkInbetweenShapeOverlap = function (line) {
        ...
        if (!(line.paragraph.containerWidget instanceof TextFrame) && line.paragraph.bodyWidget) {
            ...
            line.paragraph.bodyWidget.floatingElements.forEach(function (shape) {
                ...
                var lineRect = new Rect(line.paragraph.x, _this.viewer.clientActiveArea.y, line.paragraph.width, line.height);
    ...
    Layout.prototype.moveToNextPage = function (viewer, line, isPageBreak) {
        ...
        if (this.isRelayoutOverlap && this.endOverlapWidget) {
            var block_2 = this.endOverlapWidget.previousRenderedWidget;
            var para = line.paragraph;
            while (para) {
                para.floatingElements.forEach(function (shape) {
                    if (block_2.bodyWidget.floatingElements.indexOf(shape) !== -1) {
                        block_2.bodyWidget.floatingElements.splice(block_2.bodyWidget.floatingElements.indexOf(shape), 1);
    ...
    Layout.prototype.alignBlockElement = function (block) {
        ...
        while (previousBlock) {
            if (previousBlock instanceof ParagraphWidget) {
                ...
                var firstBlock = this.documentHelper.selection.getFirstParagraphInCell(previousBlock.childWidgets[0]);
    ...
    Layout.prototype.splitParagraph = function (paragarph, index) {
        if (index === 0) {
            return paragarph;
        }
        ...
        while (count < rowWidgets.length) {
            ...
            if (row.ownerTable.isInsideTable || (this.documentHelper.splittedCellWidgets.length === 0 && tableRowWidget.y + tableRowWidget.height + cellSpacing + this.footnoteHeight <= viewer.clientArea.bottom)) {
                ...
                if (!isNullOrUndefined(splittedWidget)) {
                    ...
                    bodyWidget = this.moveBlocksToNextPage(block instanceof ParagraphWidget ? block.previousWidget : block);
    ...
    Layout.prototype.reLayoutTable = function (block, isFootnoteReLayout) {
        ...
        //Clear Hieght for all the content
        if (this.viewer.owner.isDocumentLoaded && this.viewer.owner.editorModule) {
            var block_4 = this.documentHelper.selection.getFirstParagraphInFirstCell(currentTable);
    ...
    Layout.prototype.layoutTable = function (table, startIndex) {
        ...
        if (this.documentHelper.compatibilityMode !== 'Word2013') {
            this.viewer.clientArea.x = this.viewer.clientActiveArea.x + HelperMethods.convertPointToPixel(table.firstChild.firstChild.leftMargin);
        }
    ...
    Layout.prototype.getFootNoteWidgetsBy = function (widget, footnoteElements) {
        ...
        if (widget.bodyWidget.page.footnoteWidget) {
            for (var i = 0; i < widget.bodyWidget.page.footnoteWidget.childWidgets.length; i++) {
                /* eslint-disable-next-line max-len */
                for (var j = 0; j < footnoteElements.length; j++) {
                    if (widget.bodyWidget.page.footnoteWidget.childWidgets[i].footNoteReference === footnoteElements[j]) {
                        footWidgets.push(widget.bodyWidget.page.footnoteWidget.childWidgets[i]);
                    }
                }
            }
        }
        if ((!isNullOrUndefined(widget.previousRenderedWidget) && widget.previousRenderedWidget.bodyWidget.page.footnoteWidget)) {
            for (var i = 0; i < widget.previousRenderedWidget.bodyWidget.page.footnoteWidget.childWidgets.length; i++) {
                /* eslint-disable-next-line */
                for (var j = 0; j < footnoteElements.length; j++) {
                    if (widget.previousRenderedWidget.bodyWidget.page.footnoteWidget.childWidgets[i].footNoteReference === footnoteElements[j]) {
                        footWidgets.push(widget.previousRenderedWidget.bodyWidget.page.footnoteWidget.childWidgets[i]);
                    }
                }
            }
        }
            ...
            for (var i = 0; i < widget.bodyWidget.previousRenderedWidget.page.footnoteWidget.childWidgets.length; i++) {
                /* eslint-disable-next-line max-len */
                for (var j = 0; j < footnoteElements.length; j++) {
                    if (widget.bodyWidget.previousRenderedWidget.page.footnoteWidget.childWidgets[i].footNoteReference === footnoteElements[j]) {
                        footWidgets.push(widget.bodyWidget.previousRenderedWidget.page.footnoteWidget.childWidgets[i]);
                    }
                }
            }
        }
     ...
     * @private
     */
    Layout.prototype.getFootNoteWidgetsOf = function (widget) {
        var footnoteElements = this.getFootNotesOfBlock(widget);
        var footnoteWidgets = [];
    ...
    Layout.prototype.splitWidget = function (paragraphWidget, viewer, previousBodyWidget, index, isPageBreak, footWidget) {
        ...
        if (isNullOrUndefined(paragraphWidget.previousWidget) && footNoteInFirstLine.length > 0) {
            ...
            maxElementHeight += paragraphWidget.bodyWidget.page.footnoteWidget.childWidgets[0].height;
        }
        if (viewer.clientActiveArea.height >= maxElementHeight && !isPageBreak) {
            ...
            if (paragraphWidget.containerWidget !== nextBodyWidget) {
                ...
                if (!isNullOrUndefined(blockInfo.node)) {
                    startBlock = blockInfo.node instanceof TableRowWidget ? this.splitRow(blockInfo.node) : this.splitParagraph(blockInfo.node, parseInt(blockInfo.position.index, 10));
                    this.keepWithNext = true;
                    nextBodyWidget = this.moveBlocksToNextPage(startBlock.previousWidget);
            ...
            for (var k = 0; k < footnoteWidgets.length; k++) {
                ...
                if (footNoteIndex >= 0) {
                    ...
                    if (toBodyWidget.page.footnoteWidget.childWidgets.indexOf(footNoteWidget) < 0) {
                        var childLength = toBodyWidget.page.footnoteWidget.childWidgets.length;
                        if (childLength === 1) {
